...
```bash
gh release download v0.13.0 -R andresleecom/brauo -p "brauo-0.13.0.zip"
```
...
```bash
rm -f brauo-0.13.0.zip
zip -r brauo-0.13.0.zip manifest.json background.js content.js content.css options.html options.js popup.html popup.js shared.js voices.js icons _locales
```

### PowerShell

```powershell
Remove-Item -LiteralPath .\brauo-0.13.0.zip -Force -ErrorAction SilentlyContinue
$packageFiles = @(
  'manifest.json',
  'background.js',
  'content.js',
...
Compress-Archive -LiteralPath $packageFiles -DestinationPath .\brauo-0.13.0.zip
```

- [ ] Confirm that `brauo-0.13.0.zip` contains only `manifest.json`, `background.js`, `content.js`, `content.css`, `options.html`, `options.js`, `popup.html`, `popup.js`, `shared.js`, `voices.js`, `icons/`, and `_locales/`.
...
- [ ] Open the Package tab and upload `brauo-0.13.0.zip` as a new package version.
- [ ] Confirm that the detected version is `0.13.0`.
